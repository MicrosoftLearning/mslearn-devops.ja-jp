...
> **注:**  このラボでは、.NET Web アプリの 2 つのインスタンスを任意の 2 つの Azure リージョンにデプロイし、この 2 つの Web アプリ インスタンス間で負荷分散を行う Azure Traffic Manager の負荷分散機能を実装する、回復性に優れた構成を作成します。 次に、Azure Chaos Studio を使用して、いずれかのアプリの障害をトリガーし、負荷分散実装の回復性をテストします。

> **重要:** このラボを開始する前に、デプロイ用に 2 つの Azure リージョンを選択します。 以下、具体例に沿って説明します。
> - より良い回復性テストを行うために地理的に分離されたリージョンを選択する
> - 両方のリージョンで Azure App Service (Linux) と Azure Chaos Studio がサポートされていることを確認する
> - 選択したリージョンは、ラボ全体で使用するためメモしておく
>
> リージョン ペアの例: 米国東部と米国西部、西ヨーロッパと北ヨーロッパ、オーストラリア東部と東南アジア
...
この演習では、選択した 2 つの異なる Azure リージョン内の Azure App Service に、eShopOnWeb .NET Web アプリケーションの 2 つのインスタンスをデプロイします。

演習は次のタスクで構成されます。

- eShopOnWeb リポジトリをフォークする
- Web アプリをプライマリ リージョンにデプロイする
- Web アプリをセカンダリ リージョンにデプロイする
...
リソースをデプロイする前に、CLI コマンドで使用する公式の Azure リージョン名を特定する必要があります。 ポータルの Azure リージョン名 ("米国東部" など) は、CLI コマンドで使用される名前 ("eastus" など) とは異なります。
...
1. 次のコマンドを実行して、使用可能なすべての Azure リージョンとその正式な名前を一覧表示します。

   ```bash
   az account list-locations --query "[].{DisplayName:displayName, Name:name}" --output table
   ```

1. 出力を確認し、使用する 2 つのリージョンの **[名前]** の値をメモします。 次にいくつか一般的な例を挙げます。
   ...
   > **注:**  選択したプライマリ リージョンとセカンダリ リージョンの CLI 名を記録します。 これらの値は、デプロイ コマンドで使用します。
...
1. 次のコマンドを実行して、優先リージョンを設定し、リソース グループを作成し、Web アプリをプライマリ リージョンにデプロイします。

   > **注:**  `PRIMARY_REGION` と `PRIMARY_REGION_SUFFIX` の値を、選択したプライマリ リージョンに置き換えます。 たとえば、`eastus` と `eastus` や `westeurope` と `westeurope` などを使用します。 サフィックスはリソースの名前付けに使用され、スペースを含まない短い小文字の識別子にする必要があります。
...
1. 次のコマンドを実行して、Web アプリをセカンダリ リージョンにデプロイします。

   > **注:**  `SECONDARY_REGION` と `SECONDARY_REGION_SUFFIX` の値を、選択したセカンダリ リージョンに置き換えます。 たとえば、`westus` と `westus` や `northeurope` と `northeurope` などを使用します。 サフィックスはリソースの名前付けに使用され、スペースを含まない短い小文字の識別子にする必要があります。
...
1. デプロイが完了するまで待ちます。 この処理には数分かかる場合があります。
   ...
   > **注:**  Traffic Manager の構成に必要な両方の Web アプリの名前とリソース グループ名を記録します。

## Traffic Manager を使用してワークロードの回復性を強化する

この演習では、Azure Traffic Manager を使用して、2 つの異なる Azure リージョンにある 2 つの .NET Web アプリ インスタンスに要求を分散する、回復性のある構成を実装します。

> **注:**  ラボの目的上、プライマリ リージョンにある .NET ベースの Web アプリ eShopOnWeb のデプロイをプライマリ インスタンスと見なします。 今回このように考えるのは完全に恣意的 (かつデモンストレーションだけが目的) ですが、いずれかのエンドポイントを優先すると有益なシナリオも存在する可能性があります。
...
1. **[Traffic Manager プロファイルの作成]** ページで、次の操作を実行します。
   ...
   > **注:**  すべての要求をプライマリ リージョンの Azure App Service Web アプリで処理する必要があるという、やや恣意的な想定を反映するために、優先順位ルーティング方法を選択します。
   ...
   - **[リソース グループの場所]** ドロップダウン リストで、プライマリ リージョンとして選択したものと同じ Azure リージョンを選択します。
...
1. **[devopsfoundationstmprofile | エンドポイント]** ページで、**[+ 追加]** を選択します。
   ...
   > **注:**  最初に、Azure App Service Web アプリを表すエンドポイントをプライマリ リージョンに追加します。

1. **[エンドポイントの追加]** ペインで、次の操作を実行します。

   - **[種類]** ドロップダウン リストで **[Azure エンドポイント]** が表示されることを確認します。
   - **[名前]** テキスト ボックスに「**`DevOps Foundations web app - Primary`**」と入力します。
   - **[Enable Endpoint] (エンドポイントの有効化)** チェックボックスがオンになっていることを確認します。
   - **[ターゲット リソースの種類]** ドロップダウン リストで、**[App Service]** を選択します。
   - **[ターゲット リソース]** ドロップダウン リストのプライマリ リージョンのリソース グループ セクションで、プライマリ Azure リージョンにデプロイした App Service Web アプリの名前を選択します。
   - **[優先度]** テキスト ボックスに「**1**」と入力します。

   > **注:**  優先度の値が小さいほど、優先順位が高くなります。 この場合、すべての要求は、プライマリ リージョンの Web アプリ インスタンスが使用可能な状態である限り、そのインスタンスに送信されます。
...
1. **[devopsfoundationstmprofile | エンドポイント]** ページに戻り、**[+ 追加]** を選択します。
   ...
   > **注:**  次に、セカンダリ Azure リージョンにデプロイされたもう一方の Azure App Service Web アプリを表すエンドポイントを追加します。

1. **[エンドポイントの追加]** ペインで、次の操作を実行します。

   - **[種類]** ドロップダウン リストで **[Azure エンドポイント]** が表示されることを確認します。
   - **[名前]** テキスト ボックスに「**`DevOps Foundations web app - Secondary`**」と入力します。
   - **[Enable Endpoint] (エンドポイントの有効化)** チェックボックスがオンになっていることを確認します。
   - **[ターゲット リソースの種類]** ドロップダウン リストで、**[App Service]** を選択します。
   - **[ターゲット リソース]** ドロップダウン リストのセカンダリ リージョンのリソース グループ セクションで、セカンダリ Azure リージョンにデプロイした App Service Web アプリの名前を選択します。
...
1. 5 秒以上の間隔を空けて各呼び出しを行い (DNS キャッシュの可能性を排除するため)、同じコマンドを数回再実行します。各呼び出しの出力で、プライマリ Azure リージョン内にある Web アプリの DNS 名が参照されていることに注意してください。
...
1. **[Chaos Studio \| ターゲット]** ページで、このラボで先ほどデプロイした、プライマリ リージョンのリソース グループ内にある Azure App Service Web アプリ インスタンスを選択します。
...
1. **[Create an experiment] (実験を作成する)** ページの **[基本]** タブで、次の操作を実行します。
   ...
   - **[リージョン]** ドロップダウン リストで、プライマリ リージョンとは異なる Azure リージョン (セカンダリ リージョンや別の使用可能なリージョンなど) を選択します。

   > **注:**  任意の Azure リージョンを選択することもできますが、プライマリ リージョン内のリソースの障害をテストすることを考慮すると、別のリージョン (セカンダリ リージョンなど) を選択することをお勧めします。
...
1. **[App Services]** ページで、このラボで先ほどデプロイしたプライマリ リージョン内の Azure App Service Web アプリを選択します。
...
1. Web アプリ ページの **[要点]** セクションで、Web アプリの状態が **[停止]** と表示されていることに注意してください。
   ...
   > **注:**  次に、Traffic Manager によって、そのプロファイルを対象とする要求が、セカンダリ リージョンの App Service Web アプリを表すエンドポイントに正常にリダイレクトされているかどうかを確認します。
...
1. 5 秒以上の間隔を空けて各呼び出しを行い (DNS キャッシュの可能性を排除するため)、同じコマンドを数回再実行し、各呼び出しの出力で、セカンダリ Azure リージョンにある Web アプリの DNS 名が参照されていることを確認します。
...
1. プライマリ リージョンとセカンダリ リージョンのリソース グループ (選択したリージョンに基づいて使用した名前。たとえば、**rg-eshoponweb-eastus** と **rg-eshoponweb-westus** など) に対して同じプロセスを繰り返します。
